...
const XENV_API_KEY =
  process.env.XENV_API_KEY;

const DEV_ID =
  process.env.XENV_DEV_ID ||
  "willie-games-vm";
...
const XENV =
  "https://loremgroup.org";

const GUEST_VM_TIMEOUT_MS =
  30 * 60 * 1000;

const ACCOUNT_VM_TIMEOUT_MS =
  60 * 60 * 1000;
    ...
    try {
      const response =
        await fetch(
          ...
          `&gpu=${encodeURIComponent(
            gpu
          )}` +
          `&developer_id=${encodeURIComponent(
            DEV_ID
          )}`,
          {
            headers: {
